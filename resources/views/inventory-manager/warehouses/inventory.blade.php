            ...
            <div class=" overflow-hidden lg:p-6">
                ...
                <div class="flex flex-col lg:flex-row  lg:items-center lg:justify-between">
                    <div class="flex flex-col">
                        <h1 class="text-2xl font-bold text-zinc-900">Inventory</h1>
                        <h4 class="text-zinc-700">{{ $warehouse_name }} Warehouse</h4>
                    </div>
                    <div class="flex gap-2 mt-5 lg:mt-0">
                        ...
                        <ul class="dropdown menu w-52 rounded-box bg-zinc-50 text-zinc-600 shadow-sm" popover
                            ...
                            <li class="text-zinc-600  hover:bg-zinc-100 rounded-lg  active:bg-orange-500"><a
                                    href="{{ url('/export-incoming'). '?warehouse_id=' . $warehouseId }}">Incoming</a></li>
                            <li class="text-zinc-600  hover:bg-zinc-100 rounded-lg  active:bg-orange-500"><a
                                    href="{{ url('/export-outgoing') . '?warehouse_id=' . $warehouseId }}}}">Outgoing</a></li>
                        </ul>
                ...
                <dialog id="my_modal_5" class="modal  modal-bottom sm:modal-middle">
                    <div class="modal-box bg-zinc-50 border border-zinc-300 h-100">
                        ...
                        <h3 class="text-lg font-bold text-zinc-900">Add item to {{ $warehouse_name }} Warehouse</h3>



                        <form method="POST" action="{{ route('warehouse.inventory.add') }}" x-data="{ loading: false }"
                ...
                '{{ strtolower($item->new_mrp_code) }}'.includes(search.toLowerCase())"
                                                    ...
                                                    <span>{{ $item->item }}</span>
                                                    <span
                                                        class="text-gray-500 text-xs">{{ $item->new_mrp_code }}</span>
                                                </div>
                                            @endforeach
                                        </div>
                    ...
                    <div x-data="{ tab: 'subtotal' }">
                        ...
                        <div class="mt-5 w-full">
                            <div x-show="tab === 'subtotal'" x-transition class="flex flex-col gap-6 w-full ">
                                @include('inventory-manager.warehouses.subtotal')
                            </div>

                            <div x-show="tab === 'status'" x-transition class="flex flex-col gap-6 w-full ">
                                @include('inventory-manager.warehouses.status')
                            </div>
                 ...
                 <div x-data="{ tab: 'incoming' }">
                        ...
                        <div class="mt-5 w-full">
                            <div x-show="tab === 'incoming'" x-transition class="flex flex-col gap-6 w-full ">
                                @include('inventory-manager.warehouses.dashboard.incoming')
                            </div>

                            <div x-show="tab === 'outgoing'" x-transition class="flex flex-col gap-6 w-full ">
                                @include('inventory-manager.warehouses.dashboard.outgoing')
                            </div>
                            <div x-show="tab === 'pod-pcsi'" x-transition class="flex flex-col gap-6 w-full ">
                                @include('inventory-manager.warehouses.dashboard.pod')
                            </div>
